...
def Path_gradient(numpy_image, tensor_hr, model, attr_objective, path_interpolation_func, cuda=False):
    """
    ...
    for i in range(image_interpolation.shape[0]):
        ...
        if cuda:
            ...
            if model_name != "Temp":
                ...
                result = model(_add_batch_one(img_tensor).cuda(), _add_batch_one(tensor_hr).cuda())
            if isinstance(result, tuple):  # TODO: multi result, when index 0 is result  # due to Temp model
                result = result[0]
            target = attr_objective(result)
            target.backward()
    ...
    results_numpy = np.asarray(torch.cat(result_list, dim=0))
